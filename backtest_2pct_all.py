...
datasets = {
    '4h': "/home/parallels/wwwroot/jindouyunLH/TV 数据/BYBIT_BTCUSDT.P, 240_6b7a2.csv",
    '3h': "/home/parallels/wwwroot/jindouyunLH/TV 数据/BYBIT_BTCUSDT.P, 180_fc50c.csv",
    '2h': "/home/parallels/wwwroot/jindouyunLH/TV 数据/BYBIT_BTCUSDT.P, 120_1352a.csv",
    '1h': "/home/parallels/wwwroot/jindouyunLH/TV 数据/BYBIT_BTCUSDT.P, 60_c561c.csv",
    '1d': "/home/parallels/wwwroot/jindouyunLH/TV 数据/BYBIT_BTCUSDT.P, 1D_174c8.csv",
    '15m': "/home/parallels/wwwroot/jindouyunLH/TV 数据/BYBIT_BTCUSDT.P, 15_dfcc5.csv",
}
...
for tf_name, csv_file in datasets.items():
    print(f"\n{'='*60}")
    print(f"运行: 2% 权益风险 - {tf_name.upper()}")
    print(f"{'='*60}")
...
print("\n" + "="*80)
print("2% 权益风险 - 全周期回测汇总")
print("="*80)
print(f"{'周期':>5} | {'交易':>6} | {'胜率':>7} | {'盈亏比':>7} | {'收益率':>10} | {'回撤':>10}")
print("-"*60)
for tf_name in ['1d','4h','3h','2h','1h','15m']:
    if tf_name in results:
        s = results[tf_name]
        print(f"{tf_name:>5} | {s['total_trades']:>6} | {s['win_rate']:>6.2f}% | {s['profit_factor']:>7.2f} | {s['return_pct']:>9.2f}% | {s['max_drawdown']:>9.2f}%")
